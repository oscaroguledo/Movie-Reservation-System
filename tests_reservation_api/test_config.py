...
def test_default_values_ignore_any_real_env_file():
    ...
    assert settings.service_name == "reservation-api"
    assert settings.log_level == "INFO"
    assert settings.reservation_api_port == 8002
    assert settings.postgres_url.startswith("postgresql+asyncpg://")
    assert settings.kafka_client_id == "reservation-api"
    assert settings.kafka_consumer_group_id == "reservation-api"
...
def test_settings_actually_reads_values_from_an_env_file(tmp_path):
    ...
    env_file.write_text("LOG_LEVEL=WARNING\nRESERVATION_API_PORT=9002\n")

    settings = Settings(_env_file=str(env_file))

    assert settings.log_level == "WARNING"
    assert settings.reservation_api_port == 9002
